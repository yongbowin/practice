#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time  : 11/9/18 8:30 PM
# @Author: Yongbo Wang
# @Email : yonwang@redhat.com
# @File  : 05_linear_regression_eager_api.py
# @Desc  :
...
def mean_square_fn(model_fn, inputs, labels):
    return tf.reduce_sum(tf.pow(model_fn(inputs) - labels, 2)) / (n_samples * 2)
...
for step in range(num_steps):
    ...
    if (step + 1) % display_step == 0 or step == 0:
        ...
        # <class 'tensorflow.python.ops.resource_variable_ops.ResourceVariable'> <class 'numpy.float32'>
        print(type(W), type(W.numpy()))
